...
export default function RecoommendedBooks({
  apiUrl = process.env.NEXT_PUBLIC_RECOMMENDATION_API_URL ??
    "/api/recommendation?type=recommended&limit=5",
}: {
  apiUrl: string;
}) {
  ...
  if (loading && books.length === 0) return <div>Loading recommendations…</div>;
  if (error) return <div className="text-red-600">Error: {error}</div>;
  if (books.length === 0) return <div>No recommendations</div>;
  ...
  return (
    <section className="my-6">
      ...
      <h4 className="text-[16px] text-gray-500">We think you'll like these</h4>
